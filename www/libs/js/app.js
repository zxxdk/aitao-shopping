  ...
  .controller('cartList', function ($scope,Data) {
    ...
    */
    ...
    $scope.toggleChecked = function (item) {
      ...
      var a = 0; //统计选中的数量
      for (var i = 0; i < $scope.goodsData.length; i++) { //购物车中有未选中结算的商品，全选项即为未选中
        if ($scope.goodsData[i].checked == true) {
          a++;  //选中的个数
        }
      }
      if (a == $scope.goodsData.length) {
        $scope.selcetAll = true;
      } else {
        $scope.selcetAll = false;
      }
    ...
    $scope.checkAll = function () {  //全选，结算全部购物车中商品
      ...
      for (var i = 0; i < $scope.goodsData.length; i++) {
        $scope.goodsData[i].checked = $scope.selcetAll;
      }
      $scope.account();
    }
    ...
    $scope.editFunc = function () { //切换为编辑状态
      ...
      $scope.selcetAll = false;
      for (var i = 0; i < $scope.goodsData.length; i++) {
        $scope.goodsData[i].preChecked = $scope.goodsData[i].checked;
    ...
    $scope.okFunc = function () { //退出编辑状态
      ...
      $scope.toEdit = false;
      for (var i = 0; i < $scope.goodsData.length; i++) {
        $scope.goodsData[i].checked = $scope.goodsData[i].preChecked;
        if ($scope.goodsData[i].checked == true) {
          a++;
        }
      }

      if (a == $scope.goodsData.length && $scope.goodsData.length != 0) {
        $scope.selcetAll = true;
      }
      $scope.account();
    }
    ...
    $scope.deleteGood = function () { //编辑状态下删除被选中的商品
      if ($scope.toEdit == true && $scope.goodsData.length > 0) {
        for (var i = 0; i < $scope.goodsData.length; i++) {
          if ($scope.goodsData[i].checked == true) {
            $scope.goodsData.splice($scope.goodsData.indexOf($scope.goods[i]), 1);
    ...
    $scope.account = function () { //计算总价
      ...
      for (var i = 0; i < $scope.goodsData.length; i++) {
        ...
        if ($scope.goodsData[i].checked) {
          $scope.goodsData[i].singleTotal = $scope.goodsData[i].price * $scope.goodsData[i].count; //单个商品的总价格
          $scope.sum += $scope.goodsData[i].count; //计算总数量
        } else if ($scope.goodsData[i].checked == false) {
          $scope.goodsData[i].singleTotal = 0;
        }
        $scope.totalValue += $scope.goodsData[i].singleTotal; //计算总价
      }
    }
 })
  ...
  .factory('Data', function () { //公共数据
    return {
      ...
      goodslist: [   //商品列表
        ...
        {
          id: 11,
          name: "时尚牛皮女式手拿包 外贸韩版真皮女士零钱包手拎包包真皮手拿包",
          image: "../libs/img/23.jpg",
          price: 39.9,
          store: "淘宝",
          dimension: ["粉色", "黑色"],
          color: ["黑色", " 粉色", "红色"],
        ...
        {
          id: 12,
          name: "魅班夏季短袖印花连衣裙新款女装a字裙子潮",
          image: "../libs/img/24.jpg",
          price: 457,
          store: "淘宝",
          dimension: ["花色"],
          color: ["花色"],
        ...
        {
          id: 13,
          name: "诺比恩高靠背扶手椅 桦木贴面 特价促销",
          image: "../libs/img/25.jpg",
          price: 299,
          store: "淘宝",
          dimension: ["灰色", "黑色"],
          color: ["黑色", " 灰色", "红色"],
          version: [""],
          promotion: "购满300元，可用优惠价换购商品"
        },
        {
          id: 14,
          name: "希区瑞高端复古赫本气质ol白色蕾丝腰铅笔",
          image: "../libs/img/26.jpg",
          price: 595,
          store: "淘宝",
          dimension: ["白色", "黑色"],
          color: ["黑色","白色"],
        ...
        {
          id: 15,
          name: "韩国官网正品代购 2017春季女装新款 高帮粗跟韩范时尚百搭5204",
          image: "../libs/img/27.jpg",
          price: 345,
          store: "淘宝",
          dimension: ["粉色", "黑色"],
          color: ["黑色", " 粉色", "红色"],
        ...
        {
          id: 16,
          name: "韩版V领碎花长裙海边度假沙滩长款连衣裙女",
          image: "../libs/img/28.jpg",
          price: 245.9,
          store: "淘宝",
          dimension: ["粉色", "黑色"],
          color: ["黑色", " 粉色", "红色"],
        ...
        {
          id: 17,
          name: "烨天下 欧式洽谈桌椅实木家具售楼处影楼接待椅新古典单人沙发椅",
          image: "../libs/img/29.jpg",
          price: 1380,
          store: "淘宝",
          dimension: ["粉色", "黑色"],
          color: ["黑色", " 粉色", "红色"],
        ...
        {
          id: 18,
          name: "基美糖果色女式包单肩斜挎链条小包包女包字母手机包油蜡皮小方包",
          image: "../libs/img/30.jpg",
          price: 35.9,
          store: "淘宝",
          dimension: ["粉色", "黑色"],
          color: ["黑色", " 粉色", "红色"],
        ...
        {
          id: 19,
          name: "广州佛山深圳宜家代购 伊克尔 单人沙发 扶手椅斯科特伯 黄色",
          image: "../libs/img/31.jpg",
          price: 709,
          store: "淘宝",
          dimension: ["黄色"],
          color: ["黄色", " 粉色", "红色"],
          version: [""],
          promotion: "购满890元，可用优惠价换购商品"
        },
        {
          id: 20,
          name: "零钱包 创意小怪兽 双肩包书包款 硬币包 钥匙扣挂件",
          image: "../libs/img/32.jpg",
          price: 19.9,
          store: "淘宝",
          dimension: ["红色", "黑色","白色"],
          color: ["黑色", " 白色", "红色"],
